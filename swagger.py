...
store_base_url = 'https://petstore.swagger.io/v2'
...
def req_info(r):
    ...
    # узнаем статус-код
    if per == 3:
        print("Status Code For Store: ",r.status_code)
    else:
...
def store (orderid):
    apii_url = f"{store_base_url}/store/order/{orderid}"
    r = requests.get(apii_url)
    return r
...
# Сделал переменную per, чтобы заличать какой статус код пришёл
